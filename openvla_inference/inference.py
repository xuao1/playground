...
class OpenVLA_engine:
    ...
    def __init__(self, idx = 0):
        ...
        self.graphs = {'vit1': [torch.cuda.CUDAGraph() for i in range(self.n_replica)],
                        'vit2': [torch.cuda.CUDAGraph() for i in range(self.n_replica)],
    ...
    def generate_cuda_graphs(self):
        ...
        for graph_id in range(self.n_replica):
            with torch.cuda.graph(self.graphs['prefill'][graph_id], stream=self.streams[graph_id]):
        ...
        out, new_cache = self.models['llm'].wrapped_decoder.make_graph(self.caches['single_token'][0], seq_len = self.text_max_seq_len, kv_cache = self.new_cache1[0])
        del out
        del new_cache
        self.out2 = {}
        self.new_cache2 = {}
    ...
    def run_V_cuda_graphs(self, num_trails=1, required_sync=True, graph_id=0):
        for i in range(num_trails):
            self.graphs['vit1'][graph_id].replay()
            if required_sync:
                torch.cuda.synchronize()
            self.graphs['vit2'][graph_id].replay()
            if required_sync:
                torch.cuda.synchronize()
    ...
    def run_parallel_req_v2(self, num_trails):
        ...
        # NOTE(ZSL): The code below is not full asynchronous!!!
        # for i in range(num_trails):
        #     worker_num = args.worker_num
        #     stream_id = i%worker_num
